...
Resolution=30;% Determined using New_Determine_Res.m
stepper=100; % Maximum number of MCMC steps
N_f=1800; % Determined using Determine_N.m
Photon_weighted_Correction=0; 
...
%SF String=[top_level_dir,'\Outputs\','Analyzed_Time',datestr((datetime('today'))),'_',filename];
...
while sum(Step<stepper)>0
    ...
    parfor ksu=1:length(LocalizationsFinal)
        ...
        % the images.
%SF         if length(LocalizationsFinal{ksu})>8000
%SF             disp('Warning: you may have to many localizations for each image, consider splitting it up')
%SF         end
        ...
        if length(LocalizationsFinal{ksu})>10 && Step(ksu)<stepper %&& length(LocalizationsFinal{ksu})<8000
            ...
            if maxtemp<max(LikHood)
                ...
                if Photon_weighted_Correction==0
