...
%SF         disp(['Frac done with phase space search to split image' num2str(onwers/300)])
...
%SF if ~exist( sprintf('%s\\Outputs',top_level_dir) )
%SF     mkdir( sprintf('%s\\Outputs',top_level_dir) );
%SF end

%SF save([sprintf('%s\\Outputs',top_level_dir),'\Split_',Condition],'LocalizationsFinal','Frame_Information','addonarray','Parameters_to_split','Came_from_image','TrueLocalizations','cut1array','cut2array','cut3array','Resolution','Photons')
